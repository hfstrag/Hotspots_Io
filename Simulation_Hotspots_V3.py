...
r0_list_km = [0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.4, 12.8, 25.6, 51.2]  # liste des r0 à tester en km
...
for r0 in r0_list:
    ...
    r = np.linspace(0.0, 5*r0, nr) #on intègre le profil B_lambda(T(r)) sur un rayon égal à la taille du pixel
    dr = r[1] - r[0]
    dA_ring = 4.0 * np.pi * r * dr
    norm = d_MAJIS**2 #on normalise par l'aire du pixel (disque de rayon d_MAJIS)
    ...
    L_hotspot_avg = numer/norm  # radiance moyenne sur le hotspot
...
# ----- affichage -----
print("r0 (km)   T_eff (K)   P_tot (GW)")
for r0_km, Te, P in zip(r0_list_km, T_eff_list, P_tot_list):
    print(f"{r0_km:6.2f}  {Te:8.2f}   {P/1e9:10.3f}")
...
## Inclusion des données observées issues des papiers Davies/Veeder
